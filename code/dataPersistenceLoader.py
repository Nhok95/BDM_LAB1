...
class dataPersistenceLoader:
	...
	def __get_metadata(self, document):

		# Check if there is a date in the filename
		# if not, add the current one
		filename_date = self.__check_for_date(document['origin_filename'])
	...
	def __insert_batch(self, batch, persistent_db, target_collection):
		...
		for document in batch:
			...
			# Get the metadata
			metadata = self.__get_metadata(document)
	...
	def __insert_api_batch(self, batch, persistent_db, target_collection):
		...
		for document in batch:
			...
			schema = {}
			schema['origin_date'] = self.__check_for_date(document['origin_filename'])
			schema['ingested_time'] = date.today().strftime('%Y-%m-%d')
